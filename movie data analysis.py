...
class ImdbMovies:
    ...
    def __str__(self):
        return '''What's going on {} {}, enjoy your movie buddy'''.format(self.firstname.capitalize(),
                                                                          self.lastname.capitalize())
    ...
    def UserReview_SentimentAnalyzer(self):
        ...
        self.vectorizer = TfidfVectorizer(min_df=3, stop_words="english", sublinear_tf=True, norm='l2',
                                          ngram_range=(1, 2))
        self.final_features = self.vectorizer.fit_transform(self.features).toarray()
        self.features_train, self.features_test, self.senti_train, self.senti_test = train_test_split(
            self.final_features, self.senti, test_size=0.2, random_state=23)
    ...
    def getDataByYear(self, FirstRange, SecondRange):
        self.movieData = pd.read_sql("""select title,genre,year,audience_rating,critic_rating,budget_in_millions 
                                    ...
                                    user_review <> ''and polarity <> '' and Year BETWEEN {} and {};""".format(
            FirstRange, SecondRange), self.mydb)
    ...
    def DataAnalysis(self):
        ...
        print('***********************************************************************************************')
        print("Data is available from {} to {}.\nEnter the range so as to bring you the analysis".format(
            self.year.Min_Year[0], self.year.Max_Year[0]))
        print('***********************************************************************************************')
        try:
    ...
    def DisplayHistogram(self, movieData, column):
        column = column.title()
        LabelDictCol = {'Critic Rating': 'critic_rating', 'Audience Rating': 'audience_rating',
                        'Budget In Millions': 'budget_in_millions'}
        sns.set(style='whitegrid')
        fig, ax = plt.subplots()
    ...
    def DisplayBoxplot(self, data, column1, column2):
        ...
        LabelDictCol = {'Critic Rating': 'critic_rating', 'Audience Rating': 'audience_rating',
                        'Budget In Millions': 'budget_in_millions', 'Genre': 'genre', 'Year': 'year'}
        fig, ax = plt.subplots()
        fig.set_size_inches(11.7, 8.27)
    ...
    def getTop10(self, data):
        p = sns.factorplot(aspect=1.5, y='title', x='audience_rating',
                           data=data.sort_values(['audience_rating', 'critic_rating'], ascending=False).drop(['genre'],
                                                                                                             axis=1).drop_duplicates().head(
                               10), palette="ch:.25", kind='bar')
        p.set(xlim=(10, 100))
    ...
    def getTop10HighBudgetMovie(self, data):
        sns.factorplot(aspect=1.5, y='title', x='budget_in_millions',
                       data=data.sort_values(['budget_in_millions'], ascending=False).drop(['genre'],
                                                                                           axis=1).drop_duplicates().head(
                           10), palette="ch:.25", kind='bar')
        sns.set_style("ticks", {"xtick.major.size": 8, "ytick.major.size": 8})
        ...
        while True:
            ...
            if flag == 1 and Choice == '':
                ...
                if Choice == 's':
                    ...
                    self.optionAnalyze = {1: 'Display Top 10 Rated Movies', 2: 'Display Top 10 High Budget Movies',
                                          3: 'Display Critic Vs Audience',
                                          4: 'Distribution of Critic Vs Audience',
                                          5: 'Display Stack Distribution of Budget', 6: 'Display Boxplot',
                                          7: 'Display Barplot',
                                          8: 'Display Dashboard'}
